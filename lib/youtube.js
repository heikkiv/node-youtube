...
/**
 * Set the OAuth 2 access token for current user
 *
 * @param {String} OAuth 2 access token
 * @return {Upload}
 * @api public
 */

Upload.prototype.accessToken = function(token){
  this._token = token;
  return this;
};
...
Upload.prototype.uploadData = function(fn){
  ...
  //var url = 'http://uploads.gdata.youtube.com/feeds/api/users/' + user + '/uploads';
    var url = 'http://uploads.gdata.youtube.com/feeds/api/users/default/uploads'
  ...
  fs.stat(path, function(err, stat){
    ...
    // header
    options.headers = {
      'Authorization': (token) ? 'Bearer ' + token : 'GoogleLogin auth=' + auth
      , 'GData-Version': '2'
      , 'X-GData-Key': 'key=' + key
      , 'Accept': 'text/plain'
